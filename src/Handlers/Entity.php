...
function handle_entity_endpoint_request() {
    global $wp_query;
    ...
    if (!empty($r_id)) {
        ...
        if ($r_id === '__base_endpoint__') {
            ...
            \Mamarmite\UIDEndpoint\Renderers\render_base_endpoint_template();
            exit;
        } else {
            global $post;
            $post = \get_post($r_id);
            ...
            if ($post && $post->post_status === 'publish') {
                ...
                //render target entity
                \Mamarmite\UIDEndpoint\Renderers\render_default_template($post);
                exit;
            } else {

...
|:-------------|:-----------------------------------------------------------------------------------------------------------------------------|:---------------:|:-----------:|:------------:|  
| `/uid`       | Base du plugiciel, une index actif seulement pour les humains.                                                               |        -        |      -      |    humain    | | `/list`      | Toutes les entités en mode lecture pour les humains (avec des liens vers la page de prévisualisation et pour chaque version JSON individuellement) |        -        |      -      |    humain    | | `/list.json` | Toutes les entités en mode `json`                                                                                            |        -        |      -      |   Machine    |  
| `/preview`   | Prévisualisation des schémas                                                                                                 |       uid       |     Oui     |    humain    |  
| `/ldjson`    | Prévisualisation des schémas                                                                                                 |       uid       |     Oui     |   Machine    |  

## `/list`
Principalement pour avoir une idée de toutes les entités sur le site.
...
```json  
{  
  "@context": "http://schema.org",  "@graph": [    {      "@type": "Place",      "@id": "http://topo.art/r/t8575",      "name": "Etc."    },    {      "@type": "Organization",      "@id": "http://topo.art/r/t8486",      "name": "TOPO",      "description": "etc."    }  ]}  
```  


Il n'y a pas de :
- Vérification si les schémas sont bien remplis
...
- [x] Ajouter la redirection 303 au niveau serveur web (apache pour la version 1) automatiquement (présentement en mode manuel).
  - [x] Protection des terminaisons du plugin, surtout pour la redirection 303.
...
- [ ] Sauvegarder les UID complets dans un meta directement dans la BD.
